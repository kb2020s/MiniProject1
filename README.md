...
Test automation tools do not replace software testing *nor* should it be treated as more valuable[^3] in any organization. However, it is only there to **speed up the deployment release cycles** and assist teams to **identify any potential issues earlier** before the applications get released to production.

## Continuous Integration

By utilizing tools like Git and Docker, it allows users to have continuous integration and delivery environments for any given applications. Features like Git hooks, can also allow users to perform scripts when specific events take place inside the repository. Continuous integration can certainly allow code from different developers in your team, to be dependable[^4]. That way, it can automate the deployment process and it allows developers to deploy code from local branches to various preferred servers.
...
A fork is similar to a clone in which it is a copy of a repository. Similarly to a clone, changes can be made to this forked repository without it affecting the master repository (the original). The difference is a fork is linked to the overall repository wherein a user can ask to have their changes pushed to the original file.[^6]

### Branch 

A branch is a subset of a master wherein users can work on a separate copy in parallel to other branches and then merge these branches back to the master once complete. Typically a branch is created through **git checkout –b 'newbranch'**.[^6] Below is the example: 

![Git Branch Example](/images/branch.png)

### Commit

A **commit** is essentially saving a file. The changes made to the branch or file is saved ready to be merged to a master branch. A commit is performed through **git commit**.[^6] Below is the example: 

![Git Commit Example](/images/commit.png)

### Merge 

A merge takes changes from one branch and adds it into a different branch or to the master branch. A merge can be completed through a pull request.[^6]

### Checkout 

A checkout is switching between different branches. This can be done through **git checkout [name of branch]**.[^6] Below is the example: 

![Git Checkout Example](/images/checkout.png)

### Push

Push allows a user to submit their files to a repository. This can be done through **git push [name of master]**.[^6] Below is the example: 

![Git Push Example](/images/push.png)

### Pull 

A pull allows a user to grab files from a master or different file so that it can be edited so that you are working from the current, latest file.[^6] Below is the example: 

![Git Pull Example](/images/pull.png)

### Remote Add 

Remote Add is similar to clone in the sense that it adds a remote repository so that it can be worked on and collaborated with various users. This can be done through  **git remote add [shortname] [url]**.[^7] Below is the example: 

![Git Remote Add Example](/images/remote_add.png)

### Status

Status allows a user to see the status of a file or commit. This can be done through **git status**.[^6] Below is the example: 

![Git Status Example](/images/status.png)

### Master

The Master Branch is the main local path of the repository. In a collaboration sense, branches are created from the master branch. Each branch is worked on and then merged with the master.[^6] Below is the example: 
